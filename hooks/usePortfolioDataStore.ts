  ...
  fetchData: async () => {
    ...
    try {
      ...
      const mappedProjects = (projectsRes.data || []).map((p: any) => {
        const projectImageData = projectImages.find(
          (pi) => pi.projectId === p.id
        );
        ...
        return {
          ...
          status: p.status,
          gradient: `from-${p.gradient_from || "blue-400"} to-${
            p.gradient_to || "blue-600"
          }`,
      ...
      // Map experience data to use company logo from companies table
      const mappedExperience = (experienceRes.data || []).map((exp: any) => ({
        ...exp,
        company: exp.companies?.name || exp.company,
        company_logo_url: exp.companies?.logo_url || exp.company_logo_url,
      }));
