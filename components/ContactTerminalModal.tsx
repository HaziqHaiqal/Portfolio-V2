  ...
  return (
    <>
      <AnimatePresence>
        {isContactOpen && (
          <>
            ...
            <div className="fixed inset-0 z-[70] flex items-center justify-center px-3 sm:px-6 pt-20 pb-16 sm:pb-24 pointer-events-none">
              <motion.div
                role="dialog"
                aria-modal="true"
                aria-label="Contact terminal"
                className={`pointer-events-auto relative w-full max-w-5xl h-[680px] max-h-full overflow-hidden rounded-2xl border ${
                  isDarkMode
      ...
      <motion.button
        ...
        className={`group fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[70] h-10 w-10 sm:h-12 sm:w-12 rounded-full flex items-center justify-center border transition-colors shadow-lg ${
          isDarkMode
            ? "bg-gray-800 border-gray-700 text-emerald-400 hover:border-emerald-400/50"
            : "bg-white border-gray-200 text-emerald-600 hover:border-emerald-500/50"
        }`}
      ...
      >
        <AnimatePresence mode="wait" initial={false}>
          {isContactOpen ? (
            ...
            >
              ❯
              <span className="ml-0.5 inline-block w-[7px] h-[2px] bg-emerald-400 animate-pulse" />
            </motion.span>
          )}
        </AnimatePresence>
      </motion.button>
