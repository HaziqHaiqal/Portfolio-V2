...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "size-8 border-border bg-background hover:bg-accent hover:text-accent-foreground",
...
// Deterministic pseudo-random in [0, 1), seeded by a number. Unlike
// Math.random(), this produces the same value on the server and on the
// client for the same seed, so it's safe to use during render without a
// hydration mismatch.
...
function SidebarMenuSkeleton({
  ...
  // Random width between 50 to 90%. Seeded by useId, which React guarantees
  // is identical between the server render and the client hydration render.
  const id = useId();
